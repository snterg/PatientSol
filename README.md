...
#### Контроллер (33 теста):
- **GET /api/patients** (5 тестов) - проверка пагинации и валидации параметров
- **GET /api/patients/search** (12 тестов) - проверка всех 9 FHIR операторов
- **GET /api/patients/{id}** (2 теста) - получение по ID (успех/неудача)
- **GET /api/patients/count** (2 теста) - подсчет количества записей
- **POST /api/patients** (4 теста) - создание пациента (валидация данных)
- **PUT /api/patients/{id}** (3 теста) - обновление пациента
- **PATCH /api/patients/{id}** (2 теста) - переключение статуса активности
- **DELETE /api/patients/{id}** (2 теста) - удаление пациента
- **GET /api/patients/info** (1 тест) - получение справочников
...
| Компонент | Количество тестов | Результат |
|-----------|-------------------|-----------|
| PatientsController | 33 |  Все тесты пройдены |
| FhirDateParser | 14 |  Все тесты пройдены |
| **Всего** | **47** |  **Успешно** |

Все тесты используют InMemory Database и не требуют реального подключения к БД.
